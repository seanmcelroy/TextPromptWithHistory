...
public sealed class TextPromptWithHistory<T> : IPrompt<T>, IHasCulture
{
    ...
    public TextPromptWithHistory(string prompt, StringComparer? comparer = null)
    {
    ...
    public async Task<T> ShowAsync(IAnsiConsole console, CancellationToken cancellationToken)
    {
        ...
        return await console.RunExclusive(async () =>
        {
            ...
            while (true)
            {
                var input = await console.ReadLine(promptStyle, IsSecret, Mask, choices, History, cancellationToken).ConfigureAwait(false);
